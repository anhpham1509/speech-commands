...
export default function useRecorder() {
  ...
  useEffect(() => {
    const MAX_RECORDER_MINUTES = 0;
    const MAX_RECORDER_SECONDS = 2;
    ...
    if (recorderState.initRecording)
      recordingInterval = setInterval(() => {
        setRecorderState((prevState: Recorder) => {
          if (
            prevState.recordingMinutes === MAX_RECORDER_MINUTES &&
            prevState.recordingSeconds === MAX_RECORDER_SECONDS
          ) {
